...
(defn report [begin-progress guess end-progress]
  ...
  (if (= begin-progress end-progress)
    (if (some #{guess} end-progress)
      ...
      (println "Sorry, the word doesn not contain:" guess))
...
#_(game "hello" (shuffled-player))

(defn alpha-player []
  (choices-player letters))

#_(game "hello" (alpha-player))

(defn freq-player []
  (choices-player (seq "etaoinshrdlcumwfgypbvkjxqz")))

#_(game "hello" (freq-player))
...
(defonce available-words
  (with-open [r (jio/reader "resource/words.txt")]
    (->> (line-seq r)
         (filter #(every? valid-letter? %))
         vec)))

#_(max (map count available-words))

(defn rand-word []
  (rand-nth available-words))

#_(repeatedly 5 rand-word)
